...
class GovernoratesViewTestCase(TestCase):
    ...
    def test_view_index_file(self):
        response = self.client.get(reverse("governorates:index"))
    ...
    def test_view_has_all_html_elements_which_need_permissions(self):
        ...
        add_new_btn = parser.css_first("[aria-label='create new object']")
        ...
        row_delete_btn = parser.css_first("a[aria-label='delete object']")
        ...
        row_delete_all_btn = parser.css_first("input[aria-label='delete all objects']")
    ...
    def test_view_contains_governorate(self):
        response = self.client.get(reverse("governorates:index"))
...
class TestViewsPermission(TestCase):
    ...
    def setUpTestData(cls):
        ...
        users: dict[str, User] = {}
        for user in strings:
            user_with_view = User.objects.create_user(username=user, password=user)
        ...
        users["user_with_view_add"].user_permissions.add(add_perm, view_perm)
        users["user_with_view_change"].user_permissions.add(change_perm, view_perm)
        users["user_with_view_delete"].user_permissions.add(delete_perm, view_perm)
        users["user_with_view_export"].user_permissions.add(export_perm, view_perm)
        users["user_with_view_add_change"].user_permissions.add(
            add_perm, change_perm, view_perm
        )
    ...
    def test_view_user_has_no_permissions(self):
        self.client.login(username="user_without_view", password="user_without_view")
