...
from apps.org.models import CostCenter, CostCenterQuerySet
...
class CostCenterFilter(
    ...
    def filter_exclude_zero_net(self, queryset: CostCenterQuerySet, name, value):
        if value == ExcludeZeroNet.EXCLUDE:
            queryset = queryset.filter(~models.Q(total_amount=0))
        return queryset

    def filter_voucher_serial(self, queryset: CostCenterQuerySet, name, value):
        return self.filter_journals("voucher__voucher_serial__in", value, queryset)

    def filter_month(self, queryset: CostCenterQuerySet, name, value):
        return self.filter_journals("month__in", value, queryset)

    def filter_year(self, queryset: CostCenterQuerySet, name, value):
        return self.filter_journals("period__year__name__in", value, queryset)

    def filter_date_from(self, queryset: CostCenterQuerySet, name, value):
        return self.filter_journals("date__gte", value, queryset)

    def filter_date_to(self, queryset: CostCenterQuerySet, name, value):
