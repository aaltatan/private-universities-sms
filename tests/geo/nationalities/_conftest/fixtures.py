...
def api_keys() -> list[str]:
    return ["id", "name", "is_local", "description"]


@pytest.fixture
def index_columns() -> list[str]:
    return ["name", "is local", "employees count", "description", "options"]
...
def custom_deleter():
    class CustomDeleter(Deleter[Nationality]):
        ...
        def check_obj_executing_possibility(self, obj: Nationality) -> bool:
...
def create_objects(django_db_setup, django_db_blocker):
    with django_db_blocker.unblock():
        NationalityFactory.create_batch(100)
        yield
