...
def index_columns() -> list[str]:
    return ["name", "cost center id", "employees count", "description", "options"]
...
def custom_deleter():
    class CustomDeleter(Deleter[CostCenter]):
        ...
        def check_obj_executing_possibility(self, obj: CostCenter) -> bool:
...
def create_objects(django_db_setup, django_db_blocker):
    with django_db_blocker.unblock():
        ...
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM org_costcenter;")
