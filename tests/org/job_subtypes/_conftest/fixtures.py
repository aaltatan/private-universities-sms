...
@pytest.fixture
def job_type_model() -> JobType:
    return JobType


@pytest.fixture
def api_keys() -> list[str]:
    return ["id", "name", "description", "job_type"]


@pytest.fixture
def index_columns() -> list[str]:
    return ["name", "description", "employees count", "job type", "options"]
...
def custom_deleter():
    class CustomDeleter(Deleter[JobSubtype]):
        error_obj_msg = "error obj message"
        error_qs_msg = "error qs message"

        def check_obj_executing_possibility(self, obj: JobSubtype) -> bool:
            return False
...
def create_objects(django_db_setup, django_db_blocker):
    with django_db_blocker.unblock():
        job_types = JobTypeFactory.create_batch(10)

        for job_type in job_types:
            JobSubtypeFactory.create_batch(10, job_type=job_type)

        JobSubtypeFactory.create(name="Hama City", job_type=job_types[0])
